...
namespace Compass
{
    ...
    abstract class BlockCompass : Block
    {
        ...
        {
            meshrefs = new MultiTextureMeshRef[MAX_ANGLED_MESHES];
            ...
            meshrefs = ObjectCacheUtil.GetOrCreate(capi, key, () => {
                var refs = new MultiTextureMeshRef[MAX_ANGLED_MESHES];
                for (var angleIndex = 0; angleIndex < MAX_ANGLED_MESHES; angleIndex++)
                {
                    ...
                    refs[angleIndex] = capi.Render.UploadMultiTextureMesh(meshData);  //  fixed
                }
                return refs;
            });

            // Fix VS GUI mirror issue
        ...
        {
            if (api.Side == EnumAppSide.Client && meshrefs != null)
            {
        ...
        public override void OnBeforeRender(ICoreClientAPI capi, ItemStack compassStack, EnumItemRenderTarget renderTarget, ref ItemRenderInfo renderinfo)
        {
            ...
            if (meshrefs != null && meshrefs[bestMeshrefIndex] != null)
            {
                renderinfo.ModelRef = meshrefs[bestMeshrefIndex];  //  just reference existing mesh
            }
        }
