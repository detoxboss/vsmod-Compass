...
namespace Compass
{
    abstract class ItemBaseCompass : Item
    {
        ...
        MultiTextureMeshRef[] meshrefs;   // FIX: use MultiTextureMeshRef
        ...
        {
            meshrefs = new MultiTextureMeshRef[MAX_ANGLED_MESHES];  // FIX
            ...
            meshrefs = ObjectCacheUtil.GetOrCreate(capi, key, () =>
            {
                ...
                {
                    float angle = (float)angleIndex / MAX_ANGLED_MESHES * 360f;
                    ...
                    // FIX: UploadMultiTextureMesh instead of UploadMesh
                    meshrefs[angleIndex] = capi.Render.UploadMultiTextureMesh(meshData);
                }
                return meshrefs;
            });

            // handle weird bug in VS where GUI shapes are drawn as mirror images
        ...
        public override void OnUnloaded(ICoreAPI api)
        {
            if (api.Side == EnumAppSide.Client)
            {
                for (var meshIndex = 0; meshIndex < MAX_ANGLED_MESHES; meshIndex++)
                {
        ...
        public override void OnBeforeRender(ICoreClientAPI capi, ItemStack itemstack, EnumItemRenderTarget target, ref ItemRenderInfo renderinfo)
        {
            ...
            int bestMeshrefIndex = (int)GameMath.Mod(resolvedAngle / (Math.PI * 2) * MAX_ANGLED_MESHES + 0.5, MAX_ANGLED_MESHES);
